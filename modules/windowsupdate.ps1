...
function Disable-WindowsUpdate {
    ...
    # 4. PROTECT SERVICE SETTINGS (Phần cốt lõi giống WUB)
    Write-Host "`nDang ap dung Protect Service Settings..." -ForegroundColor Cyan
    ...
    foreach ($svc in $criticalServices) {
        ...
        if (Test-Path $regKey) {
            ...
            # Full Control cho Administrators
            ...
            # Deny Write cho SYSTEM (ngăn Medic tự bật lại)
            try {
    ...
    Write-Host "Khuyen nghi: Restart may de ap dung toi uu." -ForegroundColor Cyan
}

# ====================== ENABLE WINDOWS UPDATE (FULL RESET) ======================
function Enable-WindowsUpdate {
    Write-Host "`n--- BAT LAI WINDOWS UPDATE (Full Reset & Restore) ---" -ForegroundColor Cyan

    # 1. Xóa toàn bộ Policy
    ...
    # 2. Reset Startup Type về mặc định
    ...
    Write-Host "`n=== HOAN TAT KHÔI PHỤC WINDOWS UPDATE ===" -ForegroundColor Green
    Write-Host "Vui long RESTART MAY ngay bay gio!" -ForegroundColor Magenta
    Write-Host "Sau restart, mo services.msc (Run as Administrator) de Start Windows Update neu can." -ForegroundColor Cyan
}
